"""
CRNN CAPTCHA Solver - FULLY OPTIMIZED Training Script
Maximizes RTX 2050 + i7-12650H + 16GB RAM
...
- ✅ Persistent Workers - No respawn overhead
- ✅ Larger batch_size=64 - Better GPU utilization
- ✅ Prefetching - Load next batch while training

Expected: 3-5x faster than original script
Original: ~5-10 min/epoch → Optimized: ~1-2 min/epoch

Usage:
    1. Update BASE_DIR in __main__
    2. Place labeled images in 'dataset' folder
    3. Run: python train_crnn_optimized.py
"""
...
def train_epoch(model, train_loader, criterion, optimizer, device, idx_to_char, 
                epoch, debug=False, scaler=None, gradient_accumulation_steps=1):
    ...
    optimizer.zero_grad()  # Zero grad once at start
...
def train_crnn_model(
    ...
    """Main training function with full hardware utilization"""
    ...
    print("=" * 80)
    print(f"{'CRNN CAPTCHA TRAINER - FULLY OPTIMIZED':^80}")
    print("=" * 80)
    ...
    print(f"Gradient Accumulation: {gradient_accumulation_steps}x (effective batch={batch_size*gradient_accumulation_steps})")
    print(f"CPU Workers: {num_workers} (parallel data loading)")
    print(f"Pin Memory: {'ENABLED ✓' if pin_memory else 'DISABLED'}")
    print(f"Persistent Workers: {'ENABLED ✓' if persistent_workers else 'DISABLED'}")
    ...
    # Load dataset
    print(f"\n{'Dataset Loading':-^80}")
    dataset = CRNNDataset(image_dir, img_height, img_width, augment=False)
    ...
    print(f"Train: {train_size} (with augmentation) | Val: {val_size} (no augmentation)")
    ...
    print(f"\n{'Creating Optimized Data Loaders':-^80}")
    ...
    if scaler:
        print(f"\n✓ Mixed Precision Training ENABLED (expect 2x speedup)")
...
if __name__ == '__main__':
    ...
    # RTX 2050 (4GB) + i7-12650H + 16GB RAM
    CONFIG = {
        ...
        # Training parameters
        'batch_size': 64,  # 4x larger (was 16)
        'epochs': 150,
        ...
        'gradient_accumulation_steps': 2,  # Simulate batch_size=128
    }
